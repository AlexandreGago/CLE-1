 ...
 *     \li getVal.
 *
 *  \author Bernardo Kaluza (NMEC 97521) and
 *          Alexandre Gago (NMEC 98123)
 *          Heavily based on the code developed by Professor António Rui Borges
 */
...
static void initialization(void) {
    if (((mem = malloc(nStorePos * sizeof(unsigned char[BlockSize]))) == NULL)) {
...
void putVal(unsigned int prodId, char val[BlockSize]) {
    ...
    if ((statusProd[prodId] = pthread_cond_signal( & fifoEmpty)) != 0)
    /* let a consumer know that a value has been
                                                                                                             stored */
    {
        errno = statusProd[prodId]; /* save error in errno */
...
unsigned char* getVal(unsigned int consId) {
    unsigned char val[BlockSize]; /* retrieved value */
    ...
    if ((statusCons[consId] = pthread_cond_signal( & fifoFull)) != 0)
    /* let a producer know that a value has been
                                                                                                         retrieved */
    {
        errno = statusCons[consId]; /* save error in errno */
