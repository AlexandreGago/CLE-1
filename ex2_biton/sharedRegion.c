...
fifo_t *fifo_sorted_crescent;
fifo_t *fifo_sorted_decrescent;
...
void initializeSharedRegion(int buffersize, char *filename) {
    ...
    fifo_sorted_crescent = malloc(sizeof(fifo_t));
    fifo_sorted_decrescent = malloc(sizeof(fifo_t));
    fifo_init(fifo_unsorted, buffersize);
    fifo_init(fifo_sorted_crescent, buffersize);
    fifo_init(fifo_sorted_decrescent, buffersize);
...
fifo_t *getFifoSortedCrescent() {
    return fifo_sorted_crescent;
}
fifo_t *getFifoSortedDecrescent() {
    return fifo_sorted_decrescent;
}
...
void freeSharedRegion() {
    ...
    fifo_destroy(fifo_sorted_crescent);
    fifo_destroy(fifo_sorted_decrescent);
    free(fileName);
    free(fifo_unsorted);
    free(fifo_sorted_crescent);
    free(fifo_sorted_decrescent);
}
